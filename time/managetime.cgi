...
if (!$success)
{
print "<p>You can limit results by setting your search here.</p>";
	...
	{		
		while ($count <= $#userlist)
		{
			...
			foreach my $myuserid (@{$myuserids})
			{
				...
				{
					$options = $options . "<option value=\"$userlist[$count]->{'userid'}\" selected>$userlist[$count]->{'firstname'} $userlist[$count]->{'lastname'}</option>\n";
				}else
				{
	...
	if (@{$mystatuses})
	{
		...
		{		
			...
			{			
				if ($key == $status)
				{
					$options = $options . "<option value=\"$key\" selected>$statuses->{$key}</option>\n";
				}else
				{
